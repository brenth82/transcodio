...
- **4 container images**: `stt_image` (NeMo + CUDA), `anthropic_image` (lightweight), `flux_image` (diffusers + CUDA), `qwen_tts_image` (Qwen3-TTS source build + CUDA)
...
- `qwen_tts_image`: pinned `QwenLM/Qwen3-TTS` source install (@ `1ab0dd75353392f28a0d05d9ca960c9954b13c83`), torch 2.8, torchaudio 2.8, flash-attn 2.8.3 (`--no-build-isolation`), soundfile
...
- **qwen_tts_image**: `nvidia/cuda:12.8.0-cudnn-devel-ubuntu22.04` + Python 3.12 + pinned Qwen3-TTS source build + FFmpeg + git + libsndfile
